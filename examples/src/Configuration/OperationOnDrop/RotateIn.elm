module Configuration.OperationOnDrop.RotateIn exposing (Model, Msg, initialModel, main, subscriptions, update, view)
    ...
    case system.info draggable of
        Just { dragIndex, dropIndex } ->
            if dragIndex /= index && dropIndex /= index then
                Html.div
                    (attrs color ++ system.dropEvents index)
                    [ Html.text value ]

            else if dragIndex /= index && dropIndex == index then
                Html.div
                    (attrs sourceColor ++ system.dropEvents index)
                    [ Html.text value ]
